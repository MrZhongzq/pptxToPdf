...
> 这份文档是逐期开发过程中积累的设计说明与取舍记录，面向的是继续开发这个项目的人。
> 想直接部署使用请看 [README](../README.md)。

---

把课程 pptx 转成能直接导入 GoodNotes / OneNote 的 PDF。

**当前进度：七期（液态玻璃 UI + HTTP v1 接口 + 黑白名单）开发完成，本机可验证；按计划暂不部署，待审核通过。** 一期的占位 PDF 已被二期真实转换取代；三期的 Graph 引擎需在管理入口完成凭证配置后才可达，见下方「管理入口」一节；四期新增了管理入口（口令登录 + Azure 凭证配置）。
...
| 七 | 液态玻璃 UI 与五处界面修复 ＋ HTTP v1 转换接口 ＋ 网站黑名单与白名单语法 | 开发完成，未部署（审核通过后合并） |
...
一次请求换一份 PDF，不经浏览器、不需要轮询。

```
GET /v1/convert
      ?fileUrl=<要转换的 pptx 的公网地址>     必填
      &engine=libreoffice|graph              默认 libreoffice
      &user=<用户名>&pwd=<密码>               graph 必填
      &animations=true                       按 p:timing 展开动画分步
      &outline=true                          生成 PDF 书签大纲
      &margins=true                          页边距重映射
```

响应直接是 PDF 字节（`Content-Disposition: attachment`），浏览器里打开这个
URL 就会弹下载。

```bash
curl -OJ "http://<host>:18993/v1/convert?fileUrl=https://example.com/deck.pptx&outline=true"
```

**参数名用英文小写**，不是中文——URL 里的中文参数要经过百分号编码，各家
HTTP 客户端行为不一，调试时极易踩坑。

**凭据走查询串**会进服务器访问日志与浏览器历史，建议只在受控网络里用。

### 谁能调用

v1 受**访问白名单**约束（见下）。webui 不受影响。

### SSRF 防护

`fileUrl` 由调用方给、服务器去请求它，因此强制以下限制，且**没有开关**：

- 只允许 `http` / `https`
- 拒绝解析到环回、私有、链路本地、保留、组播、未指定地址的目标——包括
  云元数据端点 `169.254.169.254`
- **每一跳重定向都重新校验**（「302 指向内网」是最常见的绕过手法）
- 下载体积上限沿用 `PPTX2PDF_MAX_FILE_SIZE`，边下边计数
...
| | 作用范围 | 命中后 | 优先级 |
|---|---|---|---|
| **黑名单** | 网页 + v1 | 403 直接返回 | **最高**，先查它 |
| **白名单** | **只有 v1** | 放行 | 黑名单之后 |

**网页永远不受白名单影响**——即使白名单一条都没有，webui 照常使用。
也正因如此，空白名单的语义是「v1 谁也不许用」，这是一个安全的默认。
...
*.a.com||@match{api.a.com}               范围内只允许列出的子域
a.com||@no_graph                         该来源不得使用 graph 引擎
```

修饰符用 `||@` 引导，可叠加；`@except` / `@match` 的多个值用逗号分隔。
`@match` 与 `@except` 同时出现时先算 `@match`。

**不支持完整正则**：需求给出的用例全是通配与集合运算，而放开正则会引入
ReDoS——规则从数据库读，等于把服务可用性交给规则的作者。真需要时应该加一个
显式的 `re:` 前缀并配超时保护。

> 配白名单时记得把自己的解析域名加进来，避免把自己关在门外。

在 admin 面板的「访问白名单」与「网站黑名单」两个分区里管理。
