...
4. `purge_expired_ready` 真的被 `main.startup`、`pipeline.run_task` 的 `finally`、以及 `uploads.py::complete_upload`（M-1 补的第三个调用点，理由见 §4"回收动作"一节：ready 原文件是"上传"产生的，不是"转换"产生的，只挂前两个调用点时"只传不点开始"这个场景会让回收器永远够不到自己）三处调用——本清单原本漏了这一条：终审同时删掉前两处调用，跑全套测试依然全绿（这个项目专门在"函数写了但没人调用"上栽过五次，这份清单自己却在第五次栽的同一类问题上留了个洞）；复审又单独删了 `complete_upload` 那处，同样有独立测试变红。删了任何一处，用户明确要的"1 小时回收保护磁盘"都会静默变成死代码，无任何信号。
